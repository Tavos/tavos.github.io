(function () {
  ...
  function iconFile(block) {
    ...
    if (/boost|od250|808|fuzz/.test(name)) return "Device-OD.png";
    return ICON_BY_TYPE[block.type] || "Device-Utility.png";
  }
  ...
  function findSettings(block, knobs) {
    const target = normalizeName(block.name);
    if (!target) return null;
    const pools = [knobs, album.baseKnobs].filter(Boolean);
    const aliases = [target];
    if (isLoop(block)) aliases.push("tonex", "fx loop");
  ...
  function renderSettingsList(block, knobs, scene, off) {
    ...
    const group = findSettings(block, knobs);
    if (group) {
      Object.entries(group.params).slice(0, 5).forEach(([k, v]) => rows.push([k, v]));
    }
  ...
  function sceneSummary(scene) {
    ...
    const midi = midiOf(scene);
    if (midi) {
      if (midi.off) bits.push("TONEX / FX Loop bypassed (acoustic row)");
  ...
  function renderMidiBanner(scene) {
    ...
    return `<div class="midi-banner">
      <span class="midi-pill">QC MIDI Out → TONEX · <strong>${midiLine(midi)}</strong></span>
      <span class="midi-pill">Tone Model · <strong>${midi.model || "—"}</strong></span>
    </div>`;
  }
  ...
  function midiTableRows(scenes) {
    return (scenes || []).map((s) => {
      const m = midiOf(s);
      const tonex = !m ? "—" : m.off ? "Loop off" : (m.model || "—");
      const pc = !m ? "—" : m.off ? "—" : `PC ${m.pc}${m.slot ? " · " + m.slot : ""}`;
      return `<tr>
        <td>${s.id}</td><td>${s.name}</td><td>${tonex}</td><td>${pc}</td><td>${s.use}</td><td>${s.note || "—"}</td>
      </tr>`;
    }).join("");
  }
  ...
  function renderSong(song) {
    ...
    return `
      <section class="hero">
        ...
        <div>
          ...
          <div class="chips">
            ...
            <span class="chip"><strong>On load</strong> ${song.onLoad || album.onLoad || "PC + CC#12=127"}</span>
          </div>
        </div>
      </section>
      ...
      <div class="panel">
        ...
        <h3>Quad Cortex + TONEX signal chain</h3>
        <div class="scenes">
          ${(song.scenes || []).map((s) =>
        ...
        <p class="meta" style="margin-top:10px">${sceneSummary(scene)} Grey overlay on an icon means that block is bypassed. Icons from the <a href="https://quadcortex.wiki/Appendix">Quad Cortex Wiki appendix</a>.</p>
      </div>
      ...
      <div class="panel">
        <h3>QC effects + TONEX settings</h3>
        ${renderKnobs(knobs)}
      ...
      <div class="panel">
        ...
        <table class="scene-table">
          <thead><tr><th>Scene</th><th>Name</th><th>TONEX</th><th>MIDI</th><th>Use</th><th>QC changes</th></tr></thead>
  ...
  function renderOverview() {
    return `
      <section class="hero">
        <img class="cover" src="${album.art}" alt="${album.title} cover">
        <div>
          <p class="kicker">Rush · ${album.year} · Quad Cortex + TONEX Pedal</p>
          <h2>${album.title}</h2>
          ...
          <div class="chips">
            ...
            <span class="chip"><strong>On load</strong> ${album.onLoad || "PC + CC#12=127"}</span>
          </div>
        </div>
      </section>
      <div class="grid-2">
        <div class="panel prose"><h3>Guitars</h3><p>${album.guitars}</p></div>
        <div class="panel prose"><h3>Amps (TONEX Legacy)</h3><p>${album.amps}</p></div>
      </div>
      <div class="panel prose"><h3>Effects identity (Quad Cortex)</h3><p>${album.effects}</p></div>
      <div class="panel">
        <h3>Cabling — Quad Cortex FX Loop + TONEX Pedal</h3>
      ...
      ${album.midiSlots ? `<div class="panel">
        <h3>TONEX working slots used on this album</h3>
        <table class="scene-table">
          <thead><tr><th>PC</th><th>Slot</th><th>Tone Model</th><th>Job</th></tr></thead>
          <tbody>${album.midiSlots.map((s) => `<tr><td>${s.pc}</td><td>${s.slot}</td><td>${s.model}</td><td>${s.job}</td></tr>`).join("")}</tbody>
        </table>
      </div>` : ""}
      <div class="panel">
        <h3>Shared scenes</h3>
        <table class="scene-table">
          <thead><tr><th>Scene</th><th>Name</th><th>TONEX</th><th>MIDI</th><th>Use</th><th>Notes</th></tr></thead>
      ...
      <div class="panel">
        ...
        <ol class="steps">
          <li>Load the Lifeson Legacy models into TONEX banks 07–15 (see the slot table).</li>
          <li>Cable guitar → QC In, QC Send 1 → TONEX In, TONEX L/R → QC Return 1/2, QC MIDI Out → TONEX MIDI In.</li>
          <li>Build <strong>${album.presetBase}</strong> once. FX Loop is always the amp. No QC amp/cab blocks.</li>
          <li>Pick a song. Duplicate the base preset. Assign Preset MIDI Out PC on each scene footswitch (A = Scene 1).</li>
          <li>Click a scene to see which QC blocks stay on and which TONEX capture MIDI recalls.</li>
          <li>A/B against the record: confirm the TONEX name first, then attack, midrange, width, space.</li>
        </ol>
      </div>
    `;
  }
  ...
  function buildPrintSheet(song) {
    ...
    return `
      <header class="print-head">
        ...
        <div>
          <p class="kicker">Rush · ${album.year} · Quad Cortex + TONEX Pedal · Lifeson Legacy</p>
      ...
      <div class="print-chips">
        ...
        <span><strong>On load</strong> ${song.onLoad || album.onLoad || "PC + CC#12=127"}</span>
      </div>
      ...
      <section class="print-block">
        <h2>QC effects + TONEX settings</h2>
        ${renderKnobs(knobs)}
      ...
      <footer class="print-foot">Grey overlay = bypassed. FX Loop is the amp — no QC amp/cab. MIDI: QC Out → TONEX In, channel 1. Stay in Patch Bank 0.</footer>
    `;
  }
