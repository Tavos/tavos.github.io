(function () {
  ...
  function ampTweakRoots() {
    if (Array.isArray(album.ampTweakRoots) && album.ampTweakRoots.length) return album.ampTweakRoots;
    return ["uk c30 topboost", "brit 2203"];
  }

  function isAmpTweakGroup(g) {
    const root = deviceRoot(g.name);
    return ampTweakRoots().some((a) => root === a || root.startsWith(a));
  }

  /** Song-level amp overrides for this album’s merge paths. */
    ...
    if (!tweaks.length) {
      return `
        <div class="panel">
          ...
          <p class="meta">${album.ampTweakDefault || "No amp overrides — use album template defaults. See Album template."}</p>
        </div>
      `;
    }
    return `
      <div class="panel">
        <h3>Song amp tweaks</h3>
        <p class="meta">Overrides album template defaults for this song. Unlisted knobs stay at album base.</p>
        ${renderKnobs(tweaks)}
      </div>
    `;
  }
  ...
  function pathShort(pathKey) {
    const slot = (album.ampSlots || []).find((s) => String(s.path).toLowerCase() === String(pathKey).toLowerCase());
    if (slot && slot.short) return slot.short;
    if (pathKey === "a") return "A";
    if (pathKey === "b") return "B";
  ...
  function midiTableRows(scenes) {
    return (scenes || []).map((s) => {
      ...
      if (merge) {
        ...
        const b = merge.b || 0;
        const c = merge.c || 0;
        if (a >= b && a >= c) amp = pathShort("a");
        else if (b >= c) amp = pathShort("b");
        else amp = pathShort("c");
      }
  ...
  function renderOverview() {
    return `
      <section class="hero">
        <img class="cover" src="${album.art}" alt="${album.title} cover">
        <div>
          <p class="kicker">Gustavo Cerati · ${album.year} · Quad Cortex QC-only</p>
          <h2>${album.title}</h2>
      ...
      <div class="panel">
        ...
        <ol class="steps">
          ${(album.howTo || [
            "Build the two-amp merge from the album template.",
            "Cable guitar → QC Input 1 → Out 1/2 to FRFR / interface.",
            `Build <strong>${album.presetBase}</strong> once. Scenes change merge + bypass.`,
            "Pick a song. Duplicate the base preset. Assign Scenes 1–6 to footswitches.",
            "Click a scene to see active blocks and Mixer levels.",
            "A/B against the record."
          ]).map((s) => `<li>${s}</li>`).join("")}
        </ol>
      </div>
  ...
  function buildPrintSheet(song) {
    ...
    return `
      <header class="print-head">
        ...
        <div>
          <p class="kicker">Gustavo Cerati · ${album.year} · Quad Cortex QC-only · CorOS 4.1.0</p>
