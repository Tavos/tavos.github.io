  ...
  function renderOverview() {
    return `
      <section class="hero">
        <img class="cover" src="${album.art}" alt="${album.title} cover">
        <div>
          <p class="kicker">Foo Fighters · ${album.year} · Quad Cortex QC-only</p>
          <h2>${album.title}</h2>
      ...
      <div class="panel">
        ...
        <ol class="steps">
          <li>Build the three-amp split from the amp path table (ToneMaster / AC30 / JC-120).</li>
          <li>Cable guitar → QC Input 1 → Out 1/2 to FRFR / interface. No TONEX required.</li>
          <li>Build <strong>${album.presetBase}</strong> once. Stock Amp → Cab on the split. Scenes change merge + bypass.</li>
          <li>Pick a song. Duplicate the base preset. Assign Scenes 1–6 to footswitches.</li>
          <li>Click a scene to see which QC blocks stay on and which amp merge blend is active.</li>
          <li>A/B against the record: attack first, then midrange, amp blend, then space.</li>
        </ol>
      </div>
    `;
  }
  ...
  function buildPrintSheet(song) {
    ...
    return `
      <header class="print-head">
        ...
        <div>
          <p class="kicker">Foo Fighters · ${album.year} · Quad Cortex QC-only · CorOS 4.1.0</p>
      ...
      <footer class="print-foot">Grey overlay = bypassed. Stock Amp → Cab on the grid. Scenes change merge blends and FX bypass.</footer>
    `;
  }
