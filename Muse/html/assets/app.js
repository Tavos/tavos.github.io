(function () {
  ...
  function iconFile(block) {
    ...
    if (/overlord|mono synth/.test(name)) return "Device-Filter.png";
    if (/whammy|pitch/.test(name)) return "Device-Pitch.png";
  ...
  function renderOverview() {
    return `
      <section class="hero">
        <img class="cover" src="${album.art}" alt="${album.title} cover">
        <div>
          <p class="kicker">Muse · ${album.year} · Quad Cortex + TONEX (Diezel / AC30)</p>
          <h2>${album.title}</h2>
      ...
      <div class="panel">
        ...
        <ol class="steps">
          <li>Load Diezel VH4 + AC30 captures into TONEX banks 01–02 (see <code>midi_map.md</code>).</li>
          ...
          <li>A/B against the record: attack first, then midrange (1–1.5 kHz), then pitch/fuzz identity, then space.</li>
        </ol>
      </div>
    `;
  }
  ...
  function buildPrintSheet(song) {
    ...
    return `
      <header class="print-head">
        ...
        <div>
          <p class="kicker">Muse · ${album.year} · QC + TONEX · CorOS 4.1.0</p>
